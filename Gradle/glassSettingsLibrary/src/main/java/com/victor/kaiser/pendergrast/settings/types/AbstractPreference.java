...
/**
 * AbstractPreference defines all the basics for a preference.
 * Two Kinds of AbstractPreferences
 *    1) onSelect() returns true
 *         This AbstractPreference does everything based on the tap
 *    2) onSelect() returns false
 *         After onSelect(), the list from getOptions() will be displayed,
 *         and then onOptionSelected(int index) may be called if an option is picked
 */
public abstract class AbstractPreference {
	...
	/**
	 * Builds a default View with this Preference's title and image (if any)
